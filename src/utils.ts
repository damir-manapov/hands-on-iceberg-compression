...
export const humanNumber = (num: number | null | undefined): string => num?.toLocaleString("en-US").replaceAll(',', '_') ?? '';

export function makeBatches(startId: number, totalRows: number, batchRows: number): { index: number; start: number; end: number }[] {
