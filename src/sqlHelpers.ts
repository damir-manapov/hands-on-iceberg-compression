...
export function createVariantTableSQLs(cfg: TableConfig, name: string, codec: string, level?: number): string[] {
...
export function buildInsertSQL(rangeStart: number, rangeEnd: number, cfg: TableConfig, tableName: string) {
  const fq = `${cfg.catalog}.${cfg.schema}.${tableName}`;
