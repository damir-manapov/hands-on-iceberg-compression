...
async function main() {
  ...
  for (const { codec, levels } of CODECS) {
    for (const level of levels) {
      ...
      const variantTableSQLs = createVariantTableSQLs(TABLE_CONFIG, name, codec, level);
      for (const sql of variantTableSQLs) {
        await client.execute(sql);
      }
      const cpFile = path.join(LOAD.checkpointDir, `.cp_${name}.json`);
      const t0 = Date.now();
      await loadTable(client, limiter, TABLE_CONFIG, name, LOAD.startId, LOAD.totalRows, LOAD.batchRows, cpFile);
      console.log(`Load finished for ${name} in ${((Date.now()-t0)/1000).toFixed(1)}s`);

      if (LOAD.compactAfterLoad) {
        await optimizeTable(client, `${TABLE_CONFIG.catalog}.${TABLE_CONFIG.schema}.${name}`);
      }

      const exampleRow = await client.query(createSelectExamplesSQL(TABLE_CONFIG, name));
  ...
  for (const v of variants) {
    const r = await measureSizes(client, TABLE_CONFIG, v.name, v.codec, v.level);
    results.push(r);
  }
